    ...
    public class FlatnessModel : ModuleBase
    {
        ...
        public override bool ExeModule()
        {
            ...
            try
            {
                ...
                domain.FitSurfaceFirstOrder(DispImage,
                    FitMethod.ToString(), NumIterations, ClippingFactor,
                    out HTuple surfaceHandle, out HTuple fitError);

                HOperatorSet.GetImageSize(DispImage, out HTuple width, out HTuple height);

                // 从拟合结果中提取平面参数
                HOperatorSet.GetSurfaceMatchingResult(surfaceHandle,
                    "alpha", "fitted_surface", out HTuple alpha);
                HOperatorSet.GetSurfaceMatchingResult(surfaceHandle,
                    "beta", "fitted_surface", out HTuple beta);
                HOperatorSet.GetSurfaceMatchingResult(surfaceHandle,
                    "gamma", "fitted_surface", out HTuple gamma);

                Alpha = Math.Round(alpha.D, 6);
                Beta = Math.Round(beta.D, 6);
                Gamma = Math.Round(gamma.D, 6);

                HOperatorSet.GenImageSurfaceFirstOrder(out HObject planeImageObj,
                    "real", alpha, beta, gamma, height / 2, width / 2, width, height);
        ...
        private eFitMethod _FitMethod = eFitMethod.least_squares_tukey;
