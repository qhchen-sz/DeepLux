...
namespace HV.Common
{
    public class CommonMethods
    {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    if (types[i].Contains("[]"))
                    {
                        foreach (var item in Solution.Ins.SysVar)
                        {
                            ...
                            {
                                int TimesCount = 0;
                                switch (types[i].Substring(0, types[i].Length - 2))
                                {
                                    case "bool":
                                        foreach (var item2 in (List<bool>)item.Value)
                                        {
                                            SysVar.Add(new VarModel()
                                            {
                                                Name = item.Name + "[" + TimesCount + "]",
                                                DataType = types[i].Substring(0, types[i].Length - 2),
                                                Value = item2,
                                                Note = item.Note
                                            });
                                            TimesCount++;
                                        ...
                                        break;
                                    case "int":
                                        foreach (var item2 in (List<int>)item.Value)
                                        {
                                            SysVar.Add(new VarModel()
                                            {
                                                Name = item.Name + "[" + TimesCount + "]",
                                        ...
                                        break;
                                    case "double":
                                        foreach (var item2 in (List<double>)item.Value)
                                        {
                                            SysVar.Add(new VarModel()
                                            {
                                                Name = item.Name + "[" + TimesCount + "]",
                    ...
                    {
                        if(TypeSelect == "double")
                        {
                            if (types[i] == "double" || TypeSelect == "" || types[i] == "int" || types[i] == "bool")
                            {
                                SysVar.AddRange(
                                Solution.Ins.SysVar
                        ...
                        {
                            if (types[i] == TypeSelect || TypeSelect == "")
                            {
                                SysVar.AddRange(
                                Solution.Ins.SysVar
                                    .Where(o => o.DataType == types[i])
                                    ...
                                    if(item.DataType== types[i])
                                    {
                                        ...
                                        switch (types[i].Substring(0, types[i].Length - 2))
                                        {
                                            case"bool":
                                                ...
                                                {
                                                    string notetemp1 = "";
                                                    if (note.Length-1 > TimesCount)
                                                    {
                                                        notetemp1 = note[TimesCount + 1];
                                                    }
                                                    ModuleVars[j].Add(new VarModel()
                                                    {
                                                        ModuleParam = item.ModuleParam,
                                                        Name = item.Name + "[" + TimesCount + "]",
                                                        DataType = types[i].Substring(0, types[i].Length - 2),
                                                        Value = item2,
                                                        Note = notetemp1
        ...
        public static void GetGlobeModuleList(
    ModuleParam moduleParam,
    ObservableCollection<ModuleList> Modules,
    string dataType = ""
)
        {
            ...
            {
                index += Solution.Ins.ProjectList[i].ModuleList.Count();
            }
            //var pro = Solution.Ins.GetProjectById(moduleParam.ProjectID);
            //int index = pro.GetModuleIndexByName(moduleParam.ModuleName);
            List<VarModel>[] ModuleVars = new List<VarModel>[index];
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                {
                    for (int j = 0; j < item.ModuleList.Count; j++)
                    {
                        if (item.OutputMap.ContainsKey(item.ModuleList[j].ModuleParam.ModuleName))
                        {
                            ModuleVars[j].AddRange(
                                item.OutputMap[item.ModuleList[j].ModuleParam.ModuleName].Values
                                    .Where(o => o.DataType == types[i])
                                    .Select(
                                        o =>
                                            new VarModel()
                                            {
                                                ModuleParam = o.ModuleParam,
                                                Name = item.ProjectInfo.ProcessName +","+ o.Name,
                                                DataType = o.DataType,
                                                Value = o.Value,
                                                Note = o.Note
                                            }
            ...
            for (int i = 0; i < ModuleVars.Length; i++)
            {
                if (ModuleVars[i].Count > 0)
                {
                    ModuleBase moduleObj = null;
                    foreach (var item in Solution.Ins.ProjectList)
                    {
                        moduleObj = item.GetModuleByName( ModuleVars[i][0].ModuleParam.ModuleName );
                        if (moduleObj != null)
                            break;
                    }
                        if (moduleObj == null)
                    ...
                    for (m = 0; m < types.Length; m = m2 + 1)
                    {
                        ...
                        for (int l = 0; l < list2.Count; l++)
                        {
                            if (project.OutputMap.ContainsKey(list2[l]))
                            {
                                ...
                                list3.AddRange(
                                    from o in values.Where(o => o.DataType == types[l])
                                    select new VarModel
                                    {
                                        ModuleParam = o.ModuleParam,
                                        Name = o.Name,
