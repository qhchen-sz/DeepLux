namespace  HV.Script
{
    ...
    public class ExpressionScriptSupport
    {
        ...
        /// <returns>Process方法的返回结果</returns>
        public object CodeRun()
        {
            try
            {
                ...
                if (m_Compiled)
                {
                    // 使用泛型类型T调用Process方法，并确保返回类型为T
                    object result = this._objectClass.GetType().InvokeMember("Process", BindingFlags.InvokeMethod, null, this._objectClass, null);
                    return result;
                }
            }
            catch (Exception e)
            {
                throw new InvalidOperationException("Failed to execute or cast result of Process method.", e);
            }
            return default(object); // 如果没有编译或其他原因导致无法执行，返回T的默认值
        }
