...
namespace HV.Core
{
    ...
    public abstract class ModuleBase : NotifyPropertyBase
    {
        ...
        public object GetLinkValue(string var)
        {
            ...
            if (var.StartsWith("&"))
            {
                if (var.Contains("["))
                {
                    ...
                    else if(int.TryParse(text,out index))
                    {
        ...
        public object GetLinkValue(LinkVarModel var)
        {
            ...
            {
                string[] str1 = var.Text.Split(',');
                string[] str2 = str1[0].Split('.');
                string process = str2[1];
                string text = str2[0] +"."+ str1[1];
                foreach (var item in Solution.Ins.ProjectList)
                {
                    if(item.ProjectInfo.ProcessName== process)
                    {
                        result = item.GetParamByName(text).Value;
                        break;
                    }
                }
        ...
        /// 输出变量
        /// </summary>
        public void AddOutputParam(string varName, string varType, object obj, string note = "")
        {
