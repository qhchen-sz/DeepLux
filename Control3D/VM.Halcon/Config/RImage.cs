...
namespace VM.Halcon.Config
{
    ...
    public class RImage : HImage, ISerializable
    {
        ...
        public DispImage DispImage { get; set; }
        ...
        {
            if (DispImage == null)
                DispImage = new DispImage();
            HImage hImage = new HImage(fileName);

            HOperatorSet.GetImageType(hImage, out HTuple type);
            if (type == "byte")
                Type = "2D";
            else
            {
                Type = "3D";
                HOperatorSet.CountChannels(hImage, out HTuple channelsnum);
                if(channelsnum == 2)
                {
                    
                    DispImage.HeightImage = hImage.Decompose2(out HImage image2);
                    DispImage.GrayImage = image2;
                }
                else
                {
                    DispImage.HeightImage = hImage;
                }

                
            }
        }
        public RImage(IntPtr key, bool copy) : base(key, copy) { Type = "2D"; }
        public RImage(string type, int width, int height, IntPtr pixelPointer) : base(type, width, height, pixelPointer) { Type = "2D"; }
        ...
        public RImage ( DispImage  dispImage)
        {
            Type = "3D";
    ...
    public class DispImage
    {
        public DispImage()
        {
            ...
            GrayImage = new HImage();
        }
        public HImage HeightImage { get; set; }
